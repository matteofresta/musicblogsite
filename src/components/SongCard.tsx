...
export const SongCard = ({ item }: SongCardProps) => {
  const { isOpen, handleCloseMenu, handleOpenMenu } = useMenuOpen();

  const { isHovered, isPlayingSong, handleMouseEnter, handleMouseLeave } =
  ...
  return (
    <Link to={item.link} data-testid="song-card">
      <Card
      ...
      >
        ...
        {isHovered && (
          <>
            ...
            />
            <span
              className={`absolute top-2 left-2 text-white loading loading-spinner loading-md
            ...
            >
              ...
              <ul>
                <li
                  className={`absolute top-1 right-2 flex z-99 bg-gray-200/50 backdrop-blur-lg p-1 rounded-2xl ${isOpen ? "block" : "hidden"}`}
                >
                  <a
                  ...
                  >
                    <FaSpotify className="text-white text-3xl mr-1 hover:bg-gray-100 hover:text-black duration-300 p-1 m-auto rounded-full z-50" />
                  </a>
                  <a
                    ...
                    rel="noopener noreferrer"
                  >
                    <SiYoutubemusic className="text-white text-3xl hover:bg-gray-100 hover:text-black duration-300 p-1 m-auto rounded-full z-50" />
                  </a>
                  <a
                    ...
                    rel="noopener noreferrer"
                  >
                    <SiApplemusic className="text-white text-3xl hover:bg-gray-100 hover:text-black duration-300 p-1 m-auto rounded-full z-50" />
                  </a>
                  <button>
                    <IoIosClose
                      onClick={handleMenuToggle}
        ...
        <span
          className={`absolute z-50 bottom-2 left-2 bg-gray-300/20 backdrop-blur-lg rounded-xl p-4 text-bold text-white text-md ${isHovered ? "opacity-100" : "opacity-0"} transition-opacity duration-300`}
        >
          {item.songTitle}
        </span>
        <span
